...
export default function AdminOrderDetailPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  const nextStatus =
    currentIndex >= 0 && currentIndex < ORDER_STATUS_FLOW.length - 1 ? ORDER_STATUS_FLOW[currentIndex + 1] : null;
      ...
      <section data-invoice className="hidden print:block print:text-black">
        <div className="flex items-start justify-between border-b-2 border-neutral-900 pb-3">
          <div>
            ...
            <p className="text-[11px] leading-4 text-neutral-600">{STORE_INFO.address}</p>
            <p className="text-[11px] leading-4 text-neutral-600">
              {STORE_INFO.phone} · {STORE_INFO.email}
            </p>
          </div>
          <div className="text-right">
            <h2 className="text-sm font-semibold uppercase tracking-wide">Invoice</h2>
            <p className="text-[11px] leading-4">
              Invoice No: <span className="font-medium">{currentOrder.orderNumber}</span>
            </p>
            <p className="text-[11px] leading-4">
              Order Date: {formatDate(currentOrder.createdAt, { hour: "2-digit", minute: "2-digit" })}
            </p>
        ...
        <div className="mt-3 grid grid-cols-2 gap-6 text-[11px] leading-4">
          <div>
            <p className="mb-1 font-semibold uppercase tracking-wide text-neutral-500">Bill To</p>
          ...
          <div>
            <p className="mb-1 font-semibold uppercase tracking-wide text-neutral-500">Payment</p>
            <p>Method: {PAYMENT_METHOD_LABELS[currentOrder.paymentMethod]}</p>
            <p>Status: {currentOrder.paymentStatus}</p>
            <p className="mt-1">Order Status: {getOrderStatusLabel(currentOrder.status)}</p>
            {currentOrder.trackingId && (
              <p>
                Tracking: {currentOrder.courierProvider} · {currentOrder.trackingId}
              </p>
            )}
          </div>
        </div>
        ...
        <table className="mt-4 w-full border-collapse text-[11px]">
          ...
          <tbody>
            {currentOrder.items.map((item, i) => (
              <tr key={i} className="border-b border-neutral-200 break-inside-avoid">
                ...
                <td className="py-1.5 text-right tabular-nums">{formatBDT(item.price)}</td>
                <td className="py-1.5 text-right tabular-nums">{formatBDT(item.price * item.quantity)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="ml-auto mt-3 w-56 space-y-1 text-[11px]">
          <div className="flex justify-between">
            ...
            <span className="tabular-nums">{formatBDT(currentOrder.subtotal)}</span>
          </div>
          {currentOrder.discount > 0 && (
            <div className="flex justify-between">
              <span className="text-neutral-600">Discount {currentOrder.couponCode && `(${currentOrder.couponCode})`}</span>
              <span className="tabular-nums">−{formatBDT(currentOrder.discount)}</span>
            </div>
          )}
          <div className="flex justify-between">
            <span className="text-neutral-600">Delivery</span>
            <span className="tabular-nums">{formatBDT(currentOrder.deliveryCharge)}</span>
          </div>
          <div className="flex justify-between border-t-2 border-neutral-900 pt-1 text-sm font-bold">
            <span>Total</span>
            <span className="tabular-nums">{formatBDT(currentOrder.total)}</span>
          </div>
        </div>

        <p className="mt-6 border-t border-neutral-200 pt-2 text-center text-[10px] text-neutral-500">
          Thank you for your order. For questions about this invoice, contact {STORE_INFO.email}.
        </p>
      </section>
        ...
        <header className="flex min-w-0 flex-col gap-3 rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs text-muted-foreground sm:text-sm">
              Placed {formatDate(currentOrder.createdAt, { hour: "2-digit", minute: "2-digit" })} · {itemCount} item
              {itemCount === 1 ? "" : "s"}
            </p>
          </div>
        ...
        <div className="grid min-w-0 grid-cols-1 gap-4 sm:gap-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <div className="min-w-0 space-y-4 sm:space-y-5 lg:space-y-6">
            ...
            <section className="min-w-0 overflow-hidden rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
              <div className="mb-4 flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex min-w-0 flex-col gap-2 sm:flex-row">
                  <Select value={currentOrder.status} onValueChange={(v) => setStatus(v as OrderStatus)}>
                  ...
                  {nextStatus && (
                    <Button size="sm" onClick={advanceStatus} loading={updateStatus.isPending} className="w-full sm:w-auto">
                      <Send className="size-3.5" />
              ...
              <div className="min-w-0 overflow-x-auto overscroll-x-contain">
                <OrderStatusTimeline currentStatus={currentOrder.status} events={currentOrder.timeline} />
              </div>
            </section>
            ...
            <section className="min-w-0 overflow-hidden rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
              <div className="mb-4 flex min-w-0 flex-wrap items-center justify-between gap-2">
                ...
                {currentOrder.trackingId && (
                  <Button variant="ghost" size="sm" className="h-7 gap-1 text-xs text-muted-foreground" onClick={copyTrackingId}>
            ...
            <section className="min-w-0 overflow-hidden rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="shrink-0 tabular-nums">{formatBDT(currentOrder.subtotal)}</span>
                </div>
                {currentOrder.discount > 0 && (
                  <div className="flex items-center justify-between gap-4 text-success">
                    <span className="min-w-0 truncate">Discount {currentOrder.couponCode && `(${currentOrder.couponCode})`}</span>
                    <span className="shrink-0 tabular-nums">−{formatBDT(currentOrder.discount)}</span>
                  </div>
                )}
                <div className="flex items-center justify-between gap-4">
                  <span className="text-muted-foreground">Delivery</span>
                  <span className="shrink-0 tabular-nums">{formatBDT(currentOrder.deliveryCharge)}</span>
                </div>
                <Separator />
                <div className="flex items-center justify-between gap-4 text-base font-semibold">
                  <span>Total</span>
                  <span className="shrink-0 tabular-nums">{formatBDT(currentOrder.total)}</span>
                </div>
              </div>
            </section>
          ...
          <aside className="min-w-0 space-y-4 sm:space-y-5 lg:space-y-6">
            <section className="min-w-0 overflow-hidden rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
              ...
              <p className="truncate text-sm font-medium">{currentOrder.customerName}</p>
            ...
            <section className="min-w-0 overflow-hidden rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
              ...
              <p className="wrap-break-word text-sm font-medium">{PAYMENT_METHOD_LABELS[currentOrder.paymentMethod]}</p>
              <div className="mt-2">
                <PaymentStatusBadge status={currentOrder.paymentStatus} />
              </div>
            </section>
